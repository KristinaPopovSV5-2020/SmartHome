...
token = "_HR-KNXrKsQju8lK9Zfga9s9bmslpPhOwYsLuEwyz_KEEwJwRH_ZKjEUyJjZhMPGG8B1crhFiu_BYRmloOGZzw=="
org = "ftn"
...
def retrieve_simple_data():
    ...
    |> range(start: -10m)
    |> filter(fn: (r) => r._measurement == "home/foyer/dms/temperature")"""
...
def retrieve_aggregate_data():
    query = f"""from(bucket: "{bucket}")
    |> range(start: -10m)
    |> filter(fn: (r) => r._measurement == "home/foyer/dms/humidity")
    |> mean()"""
    return handle_influx_query(query)
