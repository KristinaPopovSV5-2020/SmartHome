...
def ds_callback(button_press, publish_event, dht_settings, verbose=True):
    global publish_data_counter, publish_data_limit
...
def run_ds(settings, threads, stop_event):
    if settings['simulated']:
        ...
        ds1_thread = threading.Thread(target=run_ds_simulator, args=(2, ds_callback, stop_event, publish_event, settings))
        ds1_thread.start()
